...
const FINANCE_DELIVERY_BASE_URL = normalizeText(process.env.FINANCE_DELIVERY_BASE_URL || '');
...
const buildProviderStatusCallbackUrl = async ({ channel = 'sms', mode = 'webhook', req = null } = {}) => {
  ...
  if (FINANCE_DELIVERY_BASE_URL) {
    const sanitizedBase = FINANCE_DELIVERY_BASE_URL.replace(/\/$/, '');
    return `${sanitizedBase}/api/finance/delivery/providers/${providerKey}/status`;
  }

  const protocol = normalizeText(req?.headers?.['x-forwarded-proto']) || normalizeText(req?.protocol) || 'http';
  let host = normalizeText(req?.headers?.['x-forwarded-host']) || normalizeText(req?.get?.('host')) || '';
  if (!host) {
    host = normalizeText(process.env.FINANCE_DELIVERY_HOST || process.env.HOST || '127.0.0.1');
