...
async function recomputeRegistration(registrationId) {
  ...
  const [charges, payments] = await Promise.all([
    ...
    ShortTermPayment.find({ registrationId, status: { $ne: 'void' } }).sort({ paidAt: 1, createdAt: 1 })
  ]);
  ...
  for (const p of payments) {
    ...
    usedByPayment.set(String(p._id), used);
  }
  ...
  for (const p of payments) {
    let shortfall = round(num(p.amount) - (usedByPayment.get(String(p._id)) || 0));
    if (shortfall <= 0) continue;
    const next = (p.allocations || []).map((a) => ({ chargeId: a.chargeId, amount: num(a.amount) }));
    for (const c of charges) {
      ...
      else next.push({ chargeId: c._id, amount: take });
      shortfall = round(shortfall - take);
    }
    p.allocations = next;
    p.coveredMonths = next
