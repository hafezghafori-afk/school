...
const ROLE_DEFAULT_PERMISSIONS = Object.freeze({
  ...
  instructor: [
    'manage_content',
    'view_reports',
    // چت (مستقیم/گروه صنف)، صنف‌های آنلاین (هر دو از مسیر /chat) و آرشیف ضبط جلسات، ابزارهای
    // پایه‌ای روزمرهٔ هر استاد هستند که در داشبورد استاد همیشه تبلیغ می‌شوند — نباید نیاز به
    // اعطای دستی مجوز اضافی داشته باشند، وگرنه با «دسترسی رد شد» روبه‌رو می‌شوند.
    'chat.use',
    'recordings.view'
  ],
  // نمرات/حاضری/کارخانگی/مالیِ خودِ شاگرد، تقویم، چت و آرشیف ضبط جلسات، ابزارهای پایه‌ای
  // داشبورد شاگرد هستند (همان‌هایی که همیشه به‌عنوان میانبر تبلیغ می‌شوند) — نباید نیاز به
  // اعطای دستی مجوز اضافی داشته باشند، وگرنه هر شاگرد با «دسترسی رد شد» روبه‌رو می‌شود.
  student: [
    'grades.my.view',
    'attendance.my.view',
    'homework.my.view',
  ...
  // مشاهدهٔ داشبورد والد/سرپرست، چت با استاد/مکتب و آرشیف ضبط جلسات، ابزارهای پایه‌ای
  // هر حساب والد هستند — به همان دلیل بالا نباید نیاز به اعطای دستی مجوز اضافی داشته باشند.
  parent: [
    'dashboard.view',
    'chat.use',
    'recordings.view'
