...
async function run() {
  ...
  try {
    const [courses, schoolClasses, grades] = await Promise.all([
      ...
      Grade.find({}).select('student course classId createdAt').lean()
    ]);
    ...
    for (const row of grades) {
      ...
      if (!row.classId) {
        push(issues, 'grade_missing_class_id', row, `course=${String(row.course || '')}`);
      }
      if (row.classId && !currentClass) {
        push(issues, 'grade_class_missing', row, `classId=${String(row.classId)}`);
      }
      if (row.course && !course) {
        push(issues, 'grade_course_missing', row, `course=${String(row.course)}`);
      }
      if (!resolvedClass) {
        push(issues, 'grade_unresolved_class_mapping', row, `course=${String(row.course || '')}`);
        continue;
      }
      if (row.classId && !idsEqual(row.classId, resolvedClass._id)) {
        push(issues, 'grade_class_mismatch', row, `stored=${String(row.classId)} expected=${String(resolvedClass._id)}`);
      }
      if (compatCourseId && row.course && !idsEqual(row.course, compatCourseId)) {
        push(issues, 'grade_compat_course_mismatch', row, `stored=${String(row.course)} expected=${compatCourseId}`);
      }
      if (compatCourseId && !row.course) {
        push(warnings, 'grade_missing_compat_course', row, `expected=${compatCourseId}`);
      }
    }
    ...
    console.log(`[check:grade-class-refs] ${JSON.stringify(summary)}`);

    if (issues.length) {
      console.log('[check:grade-class-refs] first issues:');
      issues.slice(0, 20).forEach((item) => {
        console.log(` - ${item.code}: ${item.id} ${item.label} ${item.detail}`.trim());
      });
      process.exitCode = 1;
      return;
    }
    ...
    if (warnings.length) {
      console.log('[check:grade-class-refs] first warnings:');
      warnings.slice(0, 20).forEach((item) => {
        console.log(` - ${item.code}: ${item.id} ${item.label} ${item.detail}`.trim());
      });
    }

    console.log('[check:grade-class-refs] ok');
  } finally {
...
run().catch((error) => {
  console.error('[check:grade-class-refs] failed:', error && error.stack ? error.stack : error);
  process.exitCode = 1;
});
