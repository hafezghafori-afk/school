...
| `P11-BE-02` | High | Backend | Completed | Add first-class restore script with safety gates. | `npm run backup:restore -- --in <dir> --dry-run` verifies manifest v2; an approved `--force` command restores raw collections and uploads only into new staged targets, never over the live targets. | `P11-BE-01` | `backend/scripts/restoreDatabase.js`, `backend/scripts/backupRestoreShared.js`, `backend/package.json` |
