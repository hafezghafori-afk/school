    ...
    <main className="student-profile-workspace" dir="rtl">
      ...
      {activeTab === 'overview' ? <section className="student-profile-section">
        <div className="student-profile-stat-grid">
          ...
          <article><span>باقیات</span><strong>{money(finance.summary?.totalDue)}</strong></article>
        </div>
      ...
      {activeTab === 'finance' ? <section className="student-profile-section"><div className="student-profile-readonly"><b>گزارش مالی فقط برای مشاهده</b><span>ثبت پرداخت، صدور یا لغو بل، تخفیف و تأیید رسید از این پرونده امکان ندارد.</span></div><div className="student-profile-stat-grid compact"><article><span>مجموع بل</span><strong>{money(finance.summary?.totalBilled)}</strong></article><article><span>مجموع پرداخت</span><strong>{money(finance.summary?.totalPaid)}</strong></article><article><span>باقیات</span><strong>{money(finance.summary?.totalDue)}</strong></article><article><span>رسید در انتظار</span><strong>{Number(finance.summary?.pendingReceiptCount || 0).toLocaleString('fa-AF')}</strong></article></div><article className="student-profile-card"><h2>بل‌ها</h2>{finance.bills?.length ? <div className="student-profile-table-wrap"><table><thead><tr><th>شماره بل</th><th>دوره</th><th>مبلغ</th><th>پرداخت</th><th>باقیات</th><th>وضعیت</th></tr></thead><tbody>{finance.bills.map((item) => <tr key={item.id}><td>{item.billNumber || '—'}</td><td>{item.periodLabel || item.term || '—'}</td><td>{money(item.amountOriginal)}</td><td>{money(item.amountPaid)}</td><td>{money(item.amountDue)}</td><td>{statusLabel(item.status)}</td></tr>)}</tbody></table></div> : <EmptyState>بل مالی ثبت نشده است.</EmptyState>}</article><article className="student-profile-card"><h2>پرداخت‌ها و رسیدها</h2>{finance.receipts?.length ? <div className="student-profile-table-wrap"><table><thead><tr><th>تاریخ</th><th>مبلغ</th><th>روش</th><th>مرجع</th><th>وضعیت</th></tr></thead><tbody>{finance.receipts.map((item) => <tr key={item.id}><td>{dateLabel(item.paidAt)}</td><td>{money(item.amount)}</td><td>{item.paymentMethod || '—'}</td><td>{item.referenceNo || '—'}</td><td>{statusLabel(item.status)}</td></tr>)}</tbody></table></div> : <EmptyState>پرداخت ثبت نشده است.</EmptyState>}</article></section> : null}
