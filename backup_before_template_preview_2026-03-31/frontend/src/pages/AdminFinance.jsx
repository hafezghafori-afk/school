...
const toFaDateTime = (value) => {
  if (!value) return '-';
  try {
    return new Date(value).toLocaleString('fa-AF-u-ca-persian');
  } catch {
    return '-';
  }
};
...
const formatFinanceTrendLabel = (bucket, mode) => {
  ...
  try {
    if (mode === 'monthly') {
      ...
      return date.toLocaleDateString('fa-AF-u-ca-persian', { month: 'short', year: '2-digit' });
    }
    return new Date(bucket).toLocaleDateString('fa-AF-u-ca-persian', mode === 'weekly'
...
const RELIEF_TYPE_UI_LABELS = {
  discount: 'ØªØ®ÙÛŒÙ',
  waiver: 'Ù…Ø¹Ø§ÙÛŒØª Ù…Ø¨Ù„ØºÛŒ',
  penalty: 'Ø¬Ø±ÛŒÙ…Ù‡',
  manual: 'ØªØ³Ù‡ÛŒÙ„ Ø¯Ø³ØªÛŒ',
  free_student: 'Ø±Ø§ÛŒÚ¯Ø§Ù† Ú©Ø§Ù…Ù„',
  scholarship_partial: 'Ø¨ÙˆØ±Ø³ÛŒÙ‡ Ø¬Ø²Ø¦ÛŒ',
  scholarship_full: 'Ø¨ÙˆØ±Ø³ÛŒÙ‡ Ú©Ø§Ù…Ù„',
  charity_support: 'Ø­Ù…Ø§ÛŒØª Ø®ÛŒØ±ÛŒÙ‡',
  sibling_discount: 'ØªØ®ÙÛŒÙ Ø®ÙˆØ§Ù‡Ø± / Ø¨Ø±Ø§Ø¯Ø±'
};

const RELIEF_COVERAGE_MODE_UI_LABELS = {
  fixed: 'Ù…Ø¨Ù„Øº Ø«Ø§Ø¨Øª',
  percent: 'Ø¯Ø±ØµØ¯ÛŒ',
  full: 'Ù¾ÙˆØ´Ø´ Ú©Ø§Ù…Ù„'
};
...
const DELIVERY_PROVIDER_REQUIRED_FIELD_LABELS = {
  ...
  statusWebhookUrl: 'Status Callback URL',
...
export default function AdminFinance() {
  ...
  const financeRole = useMemo(
    () => normalizeFinanceRole(
      ...
      normalizeFinanceRole(localStorage.getItem('adminLevel') || '', 'finance_manager')
    ),
    []
  );
  ...
  const filteredManualStudents = useMemo(() => (
    students.filter((student) => includesFinanceSearch([
      student?.name,
      student?.fullName,
      student?.email,
      student?._id,
  ...
  const filteredDiscountStudents = useMemo(() => (
    students.filter((student) => includesFinanceSearch([
      ...
      student?.guardianName,
      student?.fatherName
    ], discountStudentSearch))
  ), [students, discountStudentSearch]);
  const filteredExemptionStudents = useMemo(() => (
    students.filter((student) => includesFinanceSearch([
      student?.name,
      student?.fullName,
      student?.email,
      ...
      student?.guardianName,
      student?.fatherName
    ], exemptionStudentSearch))
  ), [students, exemptionStudentSearch]);
  const manualStudentOptions = useMemo(() => (
    filteredManualStudents.length
      ? filteredManualStudents
      : students.filter((student) => String(student?._id || '') === String(manualForm.studentId || ''))
  ), [filteredManualStudents, students, manualForm.studentId]);
  const paymentStudentOptions = useMemo(() => (
    filteredPaymentStudents.length
      ? filteredPaymentStudents
      : students.filter((student) => String(student?._id || '') === String(paymentDeskForm.studentId || ''))
  ), [filteredPaymentStudents, students, paymentDeskForm.studentId]);
  const discountStudentOptions = useMemo(() => (
    filteredDiscountStudents.length
      ? filteredDiscountStudents
      : students.filter((student) => String(student?._id || '') === String(discountForm.studentId || ''))
  ), [filteredDiscountStudents, students, discountForm.studentId]);
  const exemptionStudentOptions = useMemo(() => (
    filteredExemptionStudents.length
      ? filteredExemptionStudents
      : students.filter((student) => String(student?._id || '') === String(exemptionForm.studentId || ''))
  ), [filteredExemptionStudents, students, exemptionForm.studentId]);
  ...
  const globalFinanceSearchResults = useMemo(() => {
    const term = String(globalFinanceSearch || '').trim();
    if (!term) return [];
    ...
    return students
      .map((student) => {
        ...
        const studentBills = bills.filter((item) => getFinanceRecordStudentUserId(item) === studentUserId);
        const studentReceipts = pendingReceipts.filter((item) => getFinanceRecordStudentUserId(item) === studentUserId);
        const studentReliefs = reliefs.filter((item) => getFinanceRecordStudentUserId(item) === studentUserId);
        ...
        const directMatch = includesFinanceSearch([
          student?.name,
          student?.fullName,
          student?.email,
          student?._id,
      ...
      .slice(0, 6);
  }, [globalFinanceSearch, students, bills, pendingReceipts, reliefs]);
  ...
  const loadAll = async () => {
    ...
    try {
      const [
        ...
        fetchJson(`${API_BASE}/api/student-finance/reports/daily-cashier?date=${encodeURIComponent(cashierReportDate)}`),
      ...
      const firstStudentId = refData?.students?.[0]?._id || '';
      ...
      if (firstStudentId && !manualForm.studentId) {
        setManualForm((prev) => ({ ...prev, studentId: firstStudentId }));
      }
      if (firstStudentId && !paymentDeskForm.studentId) {
        setPaymentDeskForm((prev) => ({ ...prev, studentId: firstStudentId }));
      }
      if (firstStudentId && !discountForm.studentId) {
        setDiscountForm((prev) => ({ ...prev, studentId: firstStudentId }));
      }
      if (firstStudentId && !exemptionForm.studentId) {
        setExemptionForm((prev) => ({ ...prev, studentId: firstStudentId }));
      }
  ...
  return (
    <section className="finance-page" data-active-section={activeSection} data-form-layout={formLayoutMode}>
      ...
      <div className="finance-card finance-global-search-card">
        ...
        {globalFinanceSpotlight ? (
          <div className="finance-subcard finance-student-spotlight">
            <div className="finance-card-head">
              <div>
                <h4>{globalFinanceSpotlight.student.fullName || globalFinanceSpotlight.student.name || 'متعلم'}</h4>
              ...
              <div className="finance-search-results">
                {globalFinanceSearchResults.slice(1, 5).map((result) => (
                  <div key={result.id} className="mini-row">
                    <span>{result.student.fullName || result.student.name || 'متعلم'}</span>
      ...
      <div className="finance-control-rail">
        <div className="finance-layout-toggle" role="group" aria-label="حالت نمایش فرم‌ها">
          <button type="button" className={formLayoutMode === 'portrait' ? 'secondary is-active' : 'secondary'} onClick={() => setFormLayoutMode('portrait')}>پورتریت</button>
          <button type="button" className={formLayoutMode === 'landscape' ? 'secondary is-active' : 'secondary'} onClick={() => setFormLayoutMode('landscape')}>لندسکیپ</button>
        </div>
      ...
      <div className="finance-grid" data-finance-section="overview payments orders settings">
        <div className="finance-card" data-finance-section="overview payments" data-testid="cashier-daily-report">
          ...
          <div className="finance-summary finance-summary-compact">
            ...
            <div><span>دریافت‌شده</span><strong>{fmt(cashierReport?.summary?.totalCollected || 0)} AFN</strong></div>
          ...
          <div className="finance-grid finance-grid-tight">
            <div className="finance-subcard">
              ...
              {!cashierReport?.methodTotals?.length && <p className="muted">برای این روز پرداختی ثبت نشده است.</p>}
            </div>
            <div className="finance-subcard">
              <h4>ثبت‌کنندگان</h4>
              ...
              {!cashierReport?.cashiers?.length && <p className="muted">کاربری برای این روز ثبت نشده است.</p>}
            </div>
          </div>
        </div>
        ...
        <form className="finance-card" data-finance-section="payments" onSubmit={createDeskPayment} data-testid="finance-payment-desk">
          ...
          <select data-testid="desk-student-select" value={paymentDeskForm.studentId} onChange={(e) => {
            ...
            <div className="finance-subcard finance-student-spotlight">
              ...
              <div className="finance-subcard-list">
                ...
                {!paymentDeskFinanceSnapshot.topReliefs.length && (
                  <p className="muted">برای این متعلم و همین دامنه فعلاً تسهیل فعالی ثبت نشده است.</p>
                )}
              </div>
            </div>
          )}
        ...
        <div className="finance-card finance-spotlight-card" data-finance-section="discounts" data-testid="relief-student-spotlight">
          ...
          <div className="finance-subcard-list">
            ...
            {!reliefFocusSnapshot.topReliefs.length && (
              <p className="muted">برای همین متعلم، صنف و سال تعلیمی هنوز تسهیل فعالی ثبت نشده است.</p>
            )}
          </div>
        </div>
      ...
      <div className="finance-grid" data-finance-section="overview reports settings">
        ...
        {selectedMonthClose ? (
          <div className="finance-card" data-finance-section="overview settings reports" data-testid="month-close-snapshot-card">
            <div className="finance-card-head">
              <div>
                <h3>snapshot ماه مالی {selectedMonthClose.monthKey}</h3>
            ...
            {selectedDeliveryProviderConfig ? (
              <div className="document-delivery-history delivery-provider-status-panel" data-testid="finance-delivery-provider-status">
                ...
                <div className="receipt-meta-grid audit-meta-grid">
                  <div><span>Source</span><strong>{selectedDeliveryProviderConfig.source === 'database' ? 'Database' : 'Environment'}</strong></div>
                  <div><span>Webhook Path</span><strong>{selectedDeliveryProviderConfig.readiness?.webhookPath || '-'}</strong></div>
                  <div><span>Webhook URL</span><strong>{selectedDeliveryProviderConfig.readiness?.webhookUrl || '-'}</strong></div>
                  <div><span>Provider Callback</span><strong>{selectedDeliveryProviderConfig.readiness?.providerCallbackUrl || '-'}</strong></div>
                  <div><span>Credential Version</span><strong>v{fmt(selectedDeliveryProviderConfig.credentialVersion || 1)}</strong></div>
                  <div><span>Last Rotated</span><strong>{toFaDateTime(selectedDeliveryProviderConfig.lastRotatedAt)}</strong></div>
                  <div><span>Updated</span><strong>{toFaDateTime(selectedDeliveryProviderConfig.updatedAt)}</strong></div>
                  <div><span>By</span><strong>{selectedDeliveryProviderConfig.updatedBy?.name || '-'}</strong></div>
                </div>
                ...
                {!shouldPreviewDeliveryTemplate ? (
                  ...
                  <>
                    <div className="receipt-meta-grid audit-meta-grid">
                      ...
                      <div><span>ماه</span><strong>{deliveryTemplatePreview.sample?.monthKey || '-'}</strong></div>
                    </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="document-archive-meta">
                        ...
                        <span>{item.monthKey || 'همه ماه‌ها'}</span>
                      </div>
                      <div className="document-archive-meta">
      ...
      {printMode === 'cashier' && cashierReportPrintModel && (
        <div className="finance-print-sheet" data-testid="printable-cashier-report-sheet">
          ...
          <div className="receipt-meta-grid">
            ...
            <div><span>جمع وصول</span><strong>{fmt(cashierReportPrintModel.summary.totalCollected || 0)} AFN</strong></div>
